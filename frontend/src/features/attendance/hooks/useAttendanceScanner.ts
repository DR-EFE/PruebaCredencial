...
const computeHash = async (payload: string) => {
  try {
    ...
    console.warn('No se pudo generar hash de verificaci贸n', error);
    return '';
  }
};
      ...
      try {
        ...
        if (parsedUrl) {
          if (!isAllowedUrl(parsedUrl)) {
            setFeedback({
              ...
              message: 'Usa una credencial institucional v谩lida emitida por el IPN.',
            });
            resumeScanning(900);
            return;
          }
        ...
        if (inscripcionError) {
          ...
          setFeedback({
            ...
            message: 'Ocurri贸 un error al verificar la inscripci贸n del alumno.',
          });
          resumeScanning(1100);
          return;
        }
        ...
        if (inscripcion) {
          ...
          try {
            ...
            updateMessages.push('Inscripci髇 creada autom醫icamente');
            ...
            setFeedback({
              type: 'error',
              title: isDuplicate ? 'Alumno dado de baja' : 'Error de Inscripci髇',
              message: isDuplicate
        ...
        const updateSummary =
          updateMessages.length > 0 ? updateMessages.join(' 路 ') : null;
        ...
        const baseMessage =
          estado === 'presente'
            ...
            : `${nombreCompletoDb} lleg贸 con ${minutosTardanza} minutos de tardanza.`;
